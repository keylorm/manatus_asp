...
Namespace Orbelink.DateAndTime
    ...
    Public Class DateHandler
        ...
        Public Shared Function ToStringFromTimeSpan(ByVal theTimeSpan As TimeSpan) As String
            Dim esNegativo As Boolean = False
            If theTimeSpan.TotalMilliseconds < 0 Then
                esNegativo = True
                theTimeSpan = theTimeSpan.Negate
            End If

            Dim texto As New StringBuilder
            If theTimeSpan.Days = 1 Then
                texto.Append(Resources.DateHandler_Resources.TimeSpan_OneDay)
            Else
                texto.AppendFormat(Resources.DateHandler_Resources.TimeSpan_DaysFormat, theTimeSpan.Days)
            End If
            texto.Append(" ")
            If theTimeSpan.Hours = 1 Then
                texto.Append(Resources.DateHandler_Resources.TimeSpan_OneHour)
            Else
                texto.AppendFormat(Resources.DateHandler_Resources.TimeSpan_HoursFormat, theTimeSpan.Hours)
            End If
            texto.Append(" ")
            If theTimeSpan.Minutes = 1 Then
                texto.Append(Resources.DateHandler_Resources.TimeSpan_OneMinute)
            Else
                texto.AppendFormat(Resources.DateHandler_Resources.TimeSpan_MinutesFormat, theTimeSpan.Minutes)
            End If

            If esNegativo Then
                Return String.Format(Resources.DateHandler_Resources.TimeSpan_Ago, texto.ToString)
            Else
                Return String.Format(Resources.DateHandler_Resources.TimeSpan_Upto, texto.ToString)
            End If
        End Function
    End Class
