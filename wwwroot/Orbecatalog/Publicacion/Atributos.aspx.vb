...
Partial Class _Publicaciones_Atributos
    Inherits PageBaseClass

    Const codigo_pantalla As String = "PU-08"
    ...
    Protected Function selectAtributos() As Boolean
        ...
        dataSet = connection.executeSelect(queryBuilder.SelectQuery(Atributos_Para_Publicaciones))
        If dataSet.Tables.Count > 0 Then
            If dataSet.Tables(0).Rows.Count > 0 Then
                ...
                For counter = 0 To dg_Atributos.Items.Count - 1
                    Dim act_Atributo As Atributos_Para_Publicaciones = results(offset + counter)
    ...
    Protected Function loadAtributo(ByVal id_atributo As Integer) As Boolean
        ...
        Atributos_Para_Publicaciones.Fields.SelectAll()
        Atributos_Para_Publicaciones.Id_atributo.Where.EqualCondition(id_atributo)
        dataSet = connection.executeSelect(queryBuilder.SelectQuery(Atributos_Para_Publicaciones))

        If dataSet.Tables.Count > 0 Then
            If dataSet.Tables(0).Rows.Count > 0 Then
                ObjectBuilder.CreateObject(dataSet.Tables(0), 0, Atributos_Para_Publicaciones)

                tbx_NombreAtributo.Text = Atributos_Para_Publicaciones.Nombre.Value
                tbx_NombreAtributo.ToolTip = Atributos_Para_Publicaciones.Id_atributo.Value
                tbx_Descripcion.Text = Atributos_Para_Publicaciones.Descripcion.Value
                If Atributos_Para_Publicaciones.AutoCompletar.Value = "1" Then
    ...
    Protected Function updateAtributo(ByVal id_atributo As Integer) As Boolean
        Dim atributo As New Atributos_Para_Publicaciones
    ...
    Protected Function insertAtributo() As Boolean
        Dim atributo As Atributos_Para_Publicaciones
        Dim autoCom As Integer = 0
        If chk_AutoCompletar.Checked Then
            autoCom = 1
        End If
        Try
            atributo = New Atributos_Para_Publicaciones(tbx_NombreAtributo.Text, autoCom, tbx_Descripcion.Text)
    ...
    Protected Sub btn_BorrarSeleccionados_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_BorrarSeleccionados.Click
        ...
        For counter As Integer = 0 To dg_Atributos.Items.Count - 1
            ...
            End If
        Next
        MyMaster.mostrarMensaje("Tipos de Publicacion borrados: " & borrados, False)
        ClearInfo(Configuration.EstadoPantalla.NORMAL, True)
