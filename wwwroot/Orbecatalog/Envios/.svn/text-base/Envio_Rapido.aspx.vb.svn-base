...
Partial Class _Envio_Rapido
    Inherits PageBaseClass

    Const codigo_pantalla As String = "EV-04"
    ...
    Protected Function insertEnvio(ByVal elContenidoURL As String) As Boolean
        ...
        Try
            ...
            Else
                id_Entidad = 1
            End If
            Envio = New Envio(id_Entidad, Date.UtcNow, tbx_ReplyTo.Text, tbx_SenderName.Text, tbx_SenderEmail.Text, tbx_SubjectEnvio.Text, tbx_SMTPUsername.Text, Me.tbx_SMTPPassword.Text, Me.tbx_ServerName.Text, elContenidoURL, "", Orbelink.Helpers.Mailer.TipoEnvio.RAPIDO)
    ...
    Protected Function salvarContenidoHTML() As String
        ...
        If upl_ArchivoCorreo.HasFile Then
            Dim filename As String = upl_ArchivoCorreo.FileName
            ...
            newFilePath = Configuration.logsPath & "Mailer\" & newFileName & extension
            fileURLPath = Configuration.Config_WebSite_LocalhostRoot & "Mailer/" & newFileName & extension

            upl_ArchivoCorreo.SaveAs(newFilePath)
